...
import { listJunks } from "../graphql/queries";
import JunkProductCard from "./JunkProductCard";
...
export default function JunkProductCardCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listJunks.replaceAll("__typename", ""),
          variables,
        })
      ).data.listJunks;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "JunkProductCardCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
